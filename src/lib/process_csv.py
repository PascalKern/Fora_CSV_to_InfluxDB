...
class BaseForaMedicalRecord(object):
    _field_filter = ['_field_filter', 'date_time',
                     'period']  # Exclude this field and all properties set by the base class
...
class ForaMedicalRecord(BaseForaMedicalRecord):
    ...
    def get_measurements_and_values(self, include_empty_values: bool = False) -> List[tuple[str, float]]:
        ...
        for k, v in self.__class__._items(self):
            if k not in self._field_filter and (
                    type(v) is str or v > 0 or include_empty_values):  # Exclude all properties set by the base class
                res.append((k, v))
        return res
...
class ForaMedicalRecords(object):
    ...
    @classmethod
    def from_csv_file(cls, csv_file: pathlib.Path) -> 'ForaMedicalRecords':
        """
        ...
        with csv_file.open() as csv_data:
            ...
            for k, v in cls.HEADER_MAPPING.items():
                reader.map(k).to(v)
            return cls(reader)

    # TODO Can the reader be keept until access?! ie. do not create the list object from it
    #  immediately to save some heap space! ie. lazy / streamed access
